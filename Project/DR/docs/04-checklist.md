# 04. 주의 사항 및 검증

## ⚠️ 주의 및 확인 사항 (실무 체크리스트)

| # | 항목 | 상세 |
|---|------|------|
| 1 | **연결 IP 값 수정** | 도쿄 리전으로 이동 시 WEB·WAS에 연결된 설정 IP 값(**GCP DB 접근 제어 등**)을 반드시 수정해야 합니다. 이 부분을 놓치면 DR 인스턴스가 DB에 붙지 못합니다. |
| 2 | **DR 리전 단독 검증** | Route 53 설정 **전에** 도쿄 리전 자체(ALB 및 단독 접속)가 정상 작동하는지 먼저 확인합니다. DR이 애초에 안 뜨면 Failover는 의미가 없습니다. |
| 3 | **DNS 캐시 초기화** | 로컬 테스트·검증 시 브라우저 및 OS의 **DNS 캐시 초기화가 필수**입니다. 캐시 때문에 전환이 안 된 것처럼 보일 수 있습니다. |
| 4 | **데이터 연속성 / 세션 유지** | 크로스 리전 전환 후 **데이터 연속성 및 세션 유지**를 검증합니다. |

---

## 🧰 유용한 명령어

**Windows — DNS 캐시 초기화**
```bat
ipconfig /flushdns
```

**Linux — 리전별 수신 트래픽 확인**
```bash
# 80번 포트로 들어오는 패킷 확인 (전환 검증용)
tcpdump -i any -A port 80
```

---

## 🔎 전환 검증 흐름 요약

```mermaid
sequenceDiagram
    participant U as 사용자(PC)
    participant R as Route 53
    participant S as 서울(Main)
    participant T as 도쿄(DR)

    U->>R: easiha.com 요청
    R->>S: 평시 Primary 라우팅
    Note over S: tcpdump로 서울 수신 확인 ✅

    Note over S: 장애 발생<br/>(웹 포트 차단 시뮬레이션)
    R--xS: Health Check 실패 감지
    R->>T: Secondary(도쿄)로 자동 우회
    Note over T: tcpdump로 도쿄 수신 확인 ✅

    Note over S: 서울 리전 원상 복구
    R->>S: Primary로 정상 회귀
    Note over S: 서울 재수신 확인 ✅
```

---

## 💡 배운 점 / 회고

- **DR은 "복제"가 아니라 "전환 검증"이 핵심**이라는 걸 체감했습니다. 백업 리전을 만들어 두는 것보다, 실제 장애 상황에서 사람 개입 없이 전환되고 되돌아오는지를 검증하는 과정이 더 중요했습니다.
- **환경 종속 설정(IP·방화벽)이 전환의 숨은 실패 지점**이었습니다. 리전이 바뀌면 GCP DB 접근 제어 IP 등을 함께 갱신해야 한다는 점을 실습으로 학습했습니다.
- **DNS 캐시** 때문에 "전환이 안 되는 것처럼 보이는" 함정을 겪으며, 검증 시 캐시 초기화를 습관화하게 되었습니다.

---

⬅️ 이전: [03. 구축 과정](03-implementation.md) · 🏠 [프로젝트 홈](../README.md)
