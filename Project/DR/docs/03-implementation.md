...
flowchart LR
    ...
    D --> E["⑤ Route 53<br/>Failover 레코드 구성"]
    E --> F["⑥ hosts 파일로<br/>도메인 매핑(실습)"]
    F --> G["⑦ 장애 시뮬레이션<br/>& 전환 검증"]
```
...
- 인스턴스 생성 후 퍼블릭 IPv4 주소와 실행 상태를 확인합니다.
...
```text
#<서울-리전-IP>   easiha.com    ← 서울 리전 (평시엔 이 라인 사용)
<도쿄-리전-IP>    easiha.com    ← 도쿄 리전 (DR)
```

> 💡 인스턴스 퍼블릭 IP는 재시작 때마다 바뀌므로, 각 리전의 실제 IP로 치환해서 사용합니다. (고정하려면 Elastic IP 사용)
