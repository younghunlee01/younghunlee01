...
- **중앙 관리** — 여러 명이 같은 State를 공유해야 인프라 상태가 일관됨
- **Lock (잠금)** — 동시에 `apply`하면 State가 깨질 수 있어, 한 번에 하나만 작업하도록 잠금
- **이력 추적 / 백업** — State 버전을 남겨 문제 시 복원 가능
- **민감 정보 보호** — 원격 backend는 민감 값을 암호화해 저장

➡️ 다음: [03. 워크플로우 & 주요명령어](03-workflow.md)  
