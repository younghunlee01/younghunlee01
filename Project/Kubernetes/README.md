...
`Kubernetes` · `kubeadm` · `kubectl` · `containerd` · `etcd`

`Calico(CNI)` · `NGINX Ingress` · `NetworkPolicy` · `CoreDNS`

`PersistentVolume` · `PVC` · `hostPath` · `emptyDir`

`RBAC` · `ghcr.io` · `Ubuntu(Linux)`
